...
class DataCache {
    ...
    fileprivate func cleanupCache() {
        privateQueue.addOperation {
            ...
            var fileURLs = [URL]()
            do {
                fileURLs = try self.fileManager.contentsOfDirectory(at: self.directoryURL,
            ...
            for aFileURL in fileURLs {
                ...
                do {
                    values = try (aFileURL as NSURL).resourceValues(forKeys: properties) as! [String : AnyObject]
                } catch {
                    continue
                }

                if let modificationDate = values[URLResourceKey.contentModificationDateKey] as? Date {
            ...
            for URL in expiredFiles {
                do {
                    try self.fileManager.removeItem(at: URL as Foundation.URL)
                } catch let error as NSError {
